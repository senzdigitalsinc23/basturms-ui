...
import { z } from 'zod';
import { getPromotionCriteria, savePromotionCriteria, getSubjects, addAuditLog, fetchPromotionCriteriaFromApi } from '@/lib/store';
...
function getSubjectsForLevel(level: SchoolLevel, allSubjects: Subject[]): Subject[] {
    const targetLevels: string[] = [];

    // Map SchoolLevel to Subject['level']
    switch (level) {
        case 'Pre-School':
            targetLevels.push('Creche', 'KG');
            break;
        case 'Lower Primary':
        case 'Upper Primary':
            targetLevels.push('Primary');
            break;
        case 'JHS':
        case 'Final Year':
            targetLevels.push('JHS');
            break;
    }

    return allSubjects.filter(sub => targetLevels.includes(sub.level));
}
...
function PromotionRuleForm({ level, allSubjects }: { level: SchoolLevel, allSubjects: Subject[] }) {
    ...
    const electiveSubjects = watch(`${level}.electiveSubjects`) || [];

    const levelSubjects = getSubjectsForLevel(level, allSubjects);

    // Filter out subjects already selected as electives from the core list
    const availableCore = levelSubjects.filter(sub => !electiveSubjects.includes(sub.id));

    // Filter out subjects already selected as core from the elective list
    const availableElectives = levelSubjects.filter(sub => !compulsorySubjects.includes(sub.id));
                        ...
                        render={({ field }) => (
                            <FormItem>
                                ...
                                <FormControl>
                                    <MultiSelectPopover
                                        title="Core Subjects"
                                        options={availableCore.map(s => ({ value: s.id, label: s.name }))}
...
export function PromotionCriteriaSettings() {
    ...
    const initialDefaultValues: FormData = ALL_SCHOOL_LEVELS.reduce((acc, level) => {
        acc[level] = {
            minAverageScore: 50,
            minPassMark: 40,
            compulsorySubjects: [],
    ...
    useEffect(() => {
        const loadCriteria = async () => {
            ...
            const criteria = await fetchPromotionCriteriaFromApi();
